...
describe("API integration", () => {
    ...
    it("obras list supports filtros query", async () => {
        ...
        const response = await request(app).get("/api/v1/obras?q=Samuel&tecnicaId=1&page=1&limit=20");
        expect(response.status).toBe(200);
    ...
    it("admin token can create obra", async () => {
        ...
        const token = jwt.sign({ sub: 1, email: "admin@test.com", role: "admin", type: "access" }, process.env.JWT_ACCESS_SECRET, { expiresIn: "15m" });
    ...
    it("rejects admin create for non-admin role", async () => {
        const token = jwt.sign({ sub: 2, email: "viewer@test.com", role: "viewer", type: "access" }, process.env.JWT_ACCESS_SECRET, { expiresIn: "15m" });
        const response = await request(app)
            .post("/api/v1/obras")
            .set("Authorization", `Bearer ${token}`)
            .send({
